(function () {
  ...
  var pageScript = [
    ...
    "function isAdLabel(text){return Object.prototype.hasOwnProperty.call(adLabels,text);}",
    // Google parks page-wide <style> blocks inside the same result wrappers that
    // hold ads and AI Overviews, so removing a container outright takes its CSS
    // with it. Rehome any stylesheets into <head> before dropping the container.
    "function drop(el){",
    "if(!el||!el.parentNode)return;",
    "var sheets=el.querySelectorAll?el.querySelectorAll('style,link[rel~=\"stylesheet\"]'):[];",
    "for(var i=0;i<sheets.length;i++){if(document.head){document.head.appendChild(sheets[i]);}}",
    "el.remove();",
    "}",
    "function removeMatches(root){",
    "if(root.nodeType===1&&root.matches(selector)){drop(root);return;}",
    "var matches=root.querySelectorAll?root.querySelectorAll(selector):[];",
    "for(var i=0;i<matches.length;i++){drop(matches[i]);}",
    "}",
